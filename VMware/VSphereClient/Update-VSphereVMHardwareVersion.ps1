...
.EXAMPLE
    $Result = & "Update-VSphereVMHardwareVersion" -Server "vcenter.vsphere.local" -Name "vm_name" -Version "v11"
#>
[CmdletBinding()]
[OutputType([String])]
param (
...
process {
    ...
    try {
        ...
        if ($CurrentVersion -eq $Version) {
            Write-Verbose "VM '$($VM.Name)' has already hardware version '$Version'."
        } else {
            Write-Verbose "Upgrade hardware version on VM $($VM.Name) from '$CurrentVersion' to '$Version'."
        }

        # Upgrade version
        $Result = Set-VM -Server $Server -VM $VM -Version $Version -Confirm:$false  # Runs synchronously.
        Write-Verbose "$Result"
        Start-Sleep -Seconds 5  # Sleep to let the task complete.

        # Assert VM has new version now.
        $VM = Get-VM -Server $Server -Name $Name
        if($VM.Version -ne $Version) {
            throw "Failed to upgrade hardware version of VM '$($VM.Name)'."
        }
