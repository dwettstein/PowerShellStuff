<#
.SYNOPSIS
    Set the portgroup of an existing VM virtual network adapter.

.DESCRIPTION
    Set the portgroup of an existing VM virtual network adapter.

    File-Name:  Set-VSphereVDPortgroup.ps1
    Author:     David Wettstein
    Version:    v1.0.0

    Changelog:
                v1.0.0, 2020-11-04, David Wettstein: Refactor script and release.
                v0.0.1, 2018-03-02, David Wettstein: First implementation.
...
.EXAMPLE
    $Result = & "Set-VSphereVDPortgroup" -Server "vcenter.vsphere.local" -Name "vm_name" -PortgroupName "portgroup_name" -VDSwitchName "vdswitch_name"
#>
[CmdletBinding()]
[OutputType([String])]
param (
...
process {
    ...
    try {
        ...
        if ([String]::IsNullOrEmpty($NetworkName)) {
            $NetworkAdapterList = Get-NetworkAdapter -Server $Server -VM $VM
            if ($NetworkAdapterList.Length -eq 0) {
                throw "The VM '$($VM.Name)' has no network adapters attached. Unable to set a portgroup."
            }
            $NetworkAdapter = $NetworkAdapterList[0]
        } else {
            $NetworkAdapter = Get-NetworkAdapter -Server $Server -VM $VM -Name $NetworkName
        }

        $Portgroup = Get-VDPortgroup -Server $Server -Name $PortgroupName -VDSwitch $VDSwitchName

        $Result = Set-NetworkAdapter -NetworkAdapter $NetworkAdapter -Portgroup $Portgroup -Confirm:$false
