...
.DESCRIPTION
    ...
    File-Name:  Add-VSphereVMSnapshot.ps1
...
.EXAMPLE
    $Result = & "Add-VSphereVMSnapshot" -Server "vcenter.vsphere.local" -Name "vm_name"
#>
[CmdletBinding()]
[OutputType([String])]
param (
    ...
    [String] $SnapshotName = "Snapshot_$(Get-Date -Format 'yyyy-MM-ddTHH:mm:ss.fffzzz')"  # ISO8601
    ,
...
process {
    ...
    try {
        ...
        if (-not $RunAsync) {
            $ResultObj.Description = $Snapshot.Description
            $ResultObj.Created = Get-Date $Snapshot.Created -Format "yyyy-MM-ddTHH:mm:ss.fffzzz"  # ISO8601
            $ResultObj.Quiesced = $Snapshot.Quiesced
            $ResultObj.SizeMB = $Snapshot.SizeMB
            $ResultObj.IsCurrent = $Snapshot.IsCurrent
            $ResultObj.IsReplaySupported = $Snapshot.IsReplaySupported
            $ResultObj.PowerState = $Snapshot.PowerState
        }
