<#
.SYNOPSIS
    Get all OrganizationVdcs of a vCloud server.
...
try {
    ...
    $Page = 1
    do {
        $Endpoint = "/query?type=adminOrgVdc"
        if (-not [String]::IsNullOrEmpty($Org)) {
            $Endpoint += "&filter=(org==$Org)"
        }
        $Endpoint += "&page=$Page"
        ...
        $OrgVdcs += $Response.QueryResultRecords.AdminVdcRecord
        $Page++
    } while ($Response.QueryResultRecords.Link.rel -contains "nextPage")
    $OutputMessage = $OrgVdcs
} catch {
