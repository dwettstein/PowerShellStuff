...
try {
    ...
    if ($AcceptAllCertificates) {
        [Xml] $Response = & "$FILE_DIR\Invoke-VCloudRequest.ps1" -Server $Server -Method "GET" -Endpoint "/api/admin/edgeGateway/$EdgeGatewayId" -AuthorizationToken $AuthorizationToken -AcceptAllCertificates
    } else {
        [Xml] $Response = & "$FILE_DIR\Invoke-VCloudRequest.ps1" -Server $Server -Method "GET" -Endpoint "/api/admin/edgeGateway/$EdgeGatewayId" -AuthorizationToken $AuthorizationToken
    }
